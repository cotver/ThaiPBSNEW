...
export function StyleOneHeroCarousel({ titles }: { titles: Title[] }) {
  ...
  return (
    <section ref={heroRef} className="relative h-[clamp(620px,min(56.25vw,100vh),2160px)] overflow-hidden px-5 pb-24 sm:px-8 lg:px-10">
      ...
      {current.showHeroDetails !== false && (
        <div className="s1-blog-hero-details">
          <div className="s1-blog-hero-kicker">
            <span>Feature / {String(active + 1).padStart(2, "0")}</span>
            <i />
            <span>{titleEyebrow(current)}</span>
          </div>
          <h1>{titleInlineText(current)}</h1>
          {[current.year, current.rating, current.duration].filter(Boolean).length > 0 && (
            <div className="s1-blog-hero-meta">{[current.year, current.rating, current.duration].filter(Boolean).map((item) => <span key={item}>{item}</span>)}</div>
          )}
          {current.description && (
            <p className="s1-blog-hero-deck">{current.description}</p>
          )}
          {current.genre && (
            <p className="s1-blog-hero-genre">Filed under / {current.genre}</p>
          )}
          {current.showHeroActions !== false && (
            <div className="s1-blog-hero-actions">
              {currentIsDisabled ? (
                <span aria-disabled="true" className="s1-blog-hero-primary is-disabled">Story unavailable</span>
              ) : (
                <Link className="s1-blog-hero-primary" href={`/prototype/style-1/article/${current.slug}`}>Read the story <span>↗</span></Link>
              )}
              <SaveForLaterButton
                className="s1-blog-hero-save"
                savedClassName="s1-blog-hero-save is-saved"
                title={current}
              />
            </div>
          )}
          <div className="s1-blog-hero-dots lg:hidden">
            {titles.map((title, index) => (
              <button
