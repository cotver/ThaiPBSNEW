          ...
          return (
            <Link className={index === 0 ? "final-year-motion__card is-featured" : "final-year-motion__card"} href={titleHref(item.slug)} key={item.slug}>
              <div className="final-year-motion__image">
                {image ? <Image alt="" fill sizes={index === 0 ? "(max-width: 640px) 100vw, 50vw" : "(max-width: 640px) 50vw, 25vw"} src={image} /> : <span className={`final-year-motion__fallback bg-gradient-to-br ${item.tone}`} />}
              </div>
